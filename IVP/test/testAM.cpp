...
TEST_CASE("Adams-Moulton method", "[Adams-Moulton]")
{
    json j;
    ...
    std::ofstream outfile("./output/testAM.txt", std::ios::app);
    outfile << "!!! Adams-Moulton !!!" << std::endl;
    ...
    SECTION("test for (10.198)")
    {
        ...
        SECTION("For T1"){
            for(int p=2; p<6; p++){
                ...
                int steps = static_cast<int>(j["AM"][str]["steps"]);
                outfile << "For T1, " << str << std::endl;
                for(int i=1; i<4; i++){
                    begin = clock();
                    AdamsMoultonSolver<6> AMmethod(pow(2,i-1)*steps);
                    auto res = AMmethod.solve(U0_1, f, T1, p);
                    end = clock();
        ...
        SECTION("For T2"){
            for(int p=2; p<6; p++){
                ...
                for(int i=1; i<4; i++){
                    begin = clock();
                    AdamsMoultonSolver<6> AMmethod(pow(2,i-1)*steps);
                    auto res = AMmethod.solve(U0_2, f, T2, p);
                    end = clock();
                    outfile << "When steps = " << pow(2,i-1)*steps << ", result = " << res;
                    auto solution_err = AMmethod.solution_error(U0_2, f, T2, p);
                    outfile << "; solution error is: " << solution_err;
                    outfile << "; CPU time: " << (end-begin)*1.0/CLOCKS_PER_SEC << std::endl;
                }
            }
