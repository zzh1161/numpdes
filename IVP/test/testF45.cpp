...
TEST_CASE("Fehlberg 4(5) embedded RK method")
{
    json j;
    ...
    std::ofstream outfile("./output/testF45.txt", std::ios::app);
    outfile << "!!! Fehlberg 4(5) embedded RK !!!" << std::endl;
    ...
    SECTION("test for (10.198)")
    {
        ...
        SECTION("For T1"){
            clock_t begin, end;
            int steps = static_cast<int>(j["F45"]["steps"]);
            outfile << "For T1, " << std::endl;
            begin = clock();
            F45embedRKsolver<6> F45method(steps);
            auto res = F45method.solve(U0_1, f, T1);
            end = clock();
    ...
    SECTION("test for (10.199)")
    {
        ...
        SECTION("For T2"){
            ...
            begin = clock();
            F45embedRKsolver<6> F45method(steps);
            auto res = F45method.solve(U0_2, f, T2);
            end = clock();
            outfile << "When steps = " << steps << ", result = " << res;
            outfile << "; CPU time: " << (end-begin)*1.0/CLOCKS_PER_SEC << std::endl;
        }
