...
TEST_CASE("Adams-Bashforth method", "[Adams-Bashforth]")
{
    json j;
    ...
    std::ofstream outfile("./output/testAB.txt", std::ios::app);
    outfile << "!!! Adams-Bashforth !!!" << std::endl;
    ...
    SECTION("test for (10.198)")
    {
        ...
        SECTION("For T1"){
            for(int p=1; p<5; p++){
                ...
                int steps = static_cast<int>(j["AB"][str]["steps"]);
                outfile << "For T1, " << str << std::endl;
                for(int i=1; i<4; i++){
                    begin = clock();
                    AdamsBashforthSolver<6> ABmethd(pow(2,i-1)*steps);
                    auto res = ABmethd.solve(U0_1, f, T1, p);
                    end = clock();
        ...
        SECTION("For T2"){
            for(int p=1; p<5; p++){
                ...
                for(int i=1; i<4; i++){
                    begin = clock();
                    AdamsBashforthSolver<6> ABmethd(pow(2,i-1)*steps);
                    auto res = ABmethd.solve(U0_2, f, T2, p);
                    end = clock();
                    outfile << "When steps = " << pow(2,i-1)*steps << ", result = " << res;
                    auto solution_err = ABmethd.solution_error(U0_2, f, T2, p);
                    outfile << "; solution error is: " << solution_err;
                    outfile << "; CPU time: " << (end-begin)*1.0/CLOCKS_PER_SEC << std::endl;
                }
            }
