...
TEST_CASE("Classical RK method")
{
    ...
    std::ofstream outfile("./output/testClassRK.txt", std::ios::app);
    outfile << "!!! Classical RK !!!" << std::endl;
    ...
    SECTION("test for (10.198)")
    {
        ...
        SECTION("For T1"){
            clock_t begin, end;
            int steps = static_cast<int>(j["cRK"]["steps"]);
            outfile << "For T1, " << std::endl;
            for(int i=1; i<4; i++){
                begin = clock();
                ClassicalRKsolver<6> classRK(pow(2,i-1)*steps);
                auto res = classRK.solve(U0_1, f, T1);
                end = clock();
    ...
    SECTION("test for (10.199)")
    {
        ...
        SECTION("For T2"){
            ...
            for(int i=1; i<4; i++){
                begin = clock();
                ClassicalRKsolver<6> classRK(pow(2,i-1)*steps);
                auto res = classRK.solve(U0_2, f, T2);
                end = clock();
                outfile << "When steps = " << pow(2,i-1)*steps << ", result = " << res;
                auto solution_err = classRK.solution_error(U0_2, f, T2);
                outfile << "; solution error is: " << solution_err;
                outfile << "; CPU time: " << (end-begin)*1.0/CLOCKS_PER_SEC << std::endl;
            }
        }
