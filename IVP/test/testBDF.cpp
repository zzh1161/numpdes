...
TEST_CASE("BDF method", "[BDF]")
{
    ...
    std::ofstream outfile("./output/testBDF.txt", std::ios::app);
    outfile << "!!! BDF !!!" << std::endl;
    ...
    SECTION("test for (10.198)")
    {
        ...
        SECTION("For T1"){
            for(int p=1; p<5; p++){
                clock_t begin,end;
                std::string str("p="+std::to_string(p));
                int steps = static_cast<int>(j["BDF"][str]["steps"]);
                outfile << "For T1, " << str << std::endl;
                for(int i=1; i<4; i++){
                    begin = clock();
                    BackDiffFormulaSolver<6> BDFmethod(pow(2,i-1)*steps);
                    auto res = BDFmethod.solve(U0_1, f, T1, p);
                    end = clock();
                    outfile << "When steps = " << pow(2,i-1)*steps << ", result = " << res;
                    auto solution_err = norm(res-U0_1, 0);
                    outfile << "; solution error is: " << BDFmethod.solution_error(U0_1, f, T1, p);
    ...
    SECTION("test for (10.199)")
    {
        ...
        SECTION("For T2"){
            for(int p=1; p<5; p++){
                ...
                std::string str("p="+std::to_string(p));
                int steps = static_cast<int>(j["BDF"][str]["steps"]);
                outfile << "For T2, " << str << std::endl;
                for(int i=1; i<4; i++){
                    begin = clock();
                    BackDiffFormulaSolver<6> BDFmethod(pow(2,i-1)*steps);
                    auto res = BDFmethod.solve(U0_2, f, T2, p);
                    end = clock();
                    outfile << "When steps = " << pow(2,i-1)*steps << ", result = " << res;
                    auto solution_err = BDFmethod.solution_error(U0_2, f, T2, p);
